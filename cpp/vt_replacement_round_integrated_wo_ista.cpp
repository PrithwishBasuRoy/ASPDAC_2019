...
vector<int> sorting_by_cost_function(Graph &g,vector<int> list_of_nodes_on_path){
    ...
    ofstream myfile;
    myfile.open("slackfile_ista_slew_round_req_26_08_04.txt",ios::out|ios::app);
...
void updating_load_capacitance_for_a_particular_cell(Graph &g, int i){
            ...
            if(loadCapacitance == 0){
                ...
                if(number_of_fanouts == 1)
                    g[i].capacitance= 0.03+loadCapacitance;
                else if(number_of_fanouts >1 && number_of_fanouts <=10)
...
//             int number_of_fanouts=successor_map.size();
//             double initial_net_capacitance= 0.08;
//             double initial_net_capacitance_less_fanout= 0.08;
//             double decline_rate=0.000435;
//             int rf=  number_of_fanouts - 10;
//             if(loadCapacitance == 0){
//                 if(number_of_fanouts == 1)
//                     g[i].capacitance= 0.08;//0.028;

//                 else if(number_of_fanouts>1 && number_of_fanouts <=10)
//                     g[i].capacitance= initial_net_capacitance_less_fanout+(number_of_fanouts - 1)*0.03;
                
//                 else        
//                     g[i].capacitance=initial_net_capacitance+(rf-1)*0.001+9*0.02;

//             }else{ 
                
//                 if(number_of_fanouts == 1)
//                     g[i].capacitance= 0.08+loadCapacitance;
//                 else if(number_of_fanouts >1 && number_of_fanouts <=10)
//                      g[i].capacitance=loadCapacitance+initial_net_capacitance_less_fanout+(number_of_fanouts - 1)*0.03;
//                 else   
//                     g[i].capacitance=loadCapacitance+initial_net_capacitance+(rf-1)*0.001+9*0.03;
...
int main(int argc,char *argv[])
{
    ...
    int num;
    if(argc!=34)//rvt leakage removed 
...
//            string choice=argv[6]; 
           ...
           //double gate_parameter;
            ...
            string s_pi_r= argv[32];
            string s_pi_f= argv[33];
        ...
        vector<int> list_of_nodes_on_path;
        string node_temp;
        ifstream roundnode(argv[31]);
        while(roundnode.good())
        {
                getline(roundnode,line);
                //stringstream ss(line);
                //ss >> node_temp;
                //if(node_temp!=""){
                int node_number=::atoi(line.c_str());

                //cout<<node_number<<"xx - the node number - xx"<<endl;
                list_of_nodes_on_path.push_back(node_number);
                //}
        }
        ...
        {   
            ...
            //else if(paritymap[searchby]==1)
                g[i].delay=min(risedelaymap[i][0],falldelaymap[i][0]);
            g[i].numreplacements=0;
            g[i].critical=0;
        }
        ...
        double totalleakage=0;
            ...
            //adjusting the slew values for flops
            for(int j=0;j<vertex_count;j++)
            {
                
                //totalleakage=g[j].leakage+totalleakage;
                if(g[j].type=="PI" && j>=385){
                    g[j].slew[0] = 0.025;
                    g[j].slew[1] = 0.019;
                }
...
//              //The markednode stores the ith node from tempvect.
                ...
                cout<<"Temp Arrival Time "<<arrival_time(maxlevel,vertex_count,g,2);
                    ...
                    if(g[markednode].numreplacements==0 && (g[markednode].slack>max(risedelaymap[markednode][1],falldelaymap[markednode][1])-g[markednode].delay))
                    {
                       ...
                       for(it_slew=list_of_nodes_on_path.begin();it_slew!=list_of_nodes_on_path.end();++it_slew){
                        int i=*it_slew;
                        g[i].slew_old[0]=g[i].slew[0];
                        g[i].slew_old[1]=g[i].slew[1];
                        // if(g[i].lvt_hvt == 1){
                        //     g[i].lvt_hvt = 2;
                        //    //slew_calculation(lvt_rise_transition_name_index_1,lvt_rise_transition_name_index_2,lvt_fall_transition_name_index_1,lvt_fall_transition_name_index_2,lvt_risetransitionmap,lvt_falltransitionmap,0, maxlevel,vertex_count,g,lvt_gate_fp_map,hvt_gate_fp_map,i);                            
                        ...
                        //     g[i].lvt_hvt = 1;
                        // } else if(i==markednode){//g[i].lvt_hvt == 2){
                        //     slew_calculation(hvt_rise_transition_name_index_1,hvt_rise_transition_name_index_2,hvt_fall_transition_name_index_1,hvt_fall_transition_name_index_2,hvt_risetransitionmap,hvt_falltransitionmap,0, maxlevel,vertex_count,g,lvt_gate_fp_map,hvt_gate_fp_map,i);                                 
                        // }

                        if(i==markednode){//g[i].lvt_hvt == 2){
                           slew_calculation(hvt_rise_transition_name_index_1,hvt_rise_transition_name_index_2,hvt_fall_transition_name_index_1,hvt_fall_transition_name_index_2,hvt_risetransitionmap,hvt_falltransitionmap,0, maxlevel,vertex_count,g,lvt_gate_fp_map,hvt_gate_fp_map,i);                                 
                        }
                        else {
                            //g[i].lvt_hvt = 2;
    ...
    //        slew_calculation(hvt_index_1,index_2,hvt_risetransitionmap,hvt_falltransitionmap,1, templevel,num1,g);
            // delay_calculation(hvt_index_rise_1,hvt_index_2,hvt_risemap,hvt_fallmap,0,templevel,num1,2,g,2,lvt_gate_fp_map,hvt_gate_fp_map);
                       ...
                       //slew_calculation(0, maxlevel,num1,g,lvt_gate_fp_map,hvt_gate_fp_map,markednode);
...
//            //cout<<"doing delay"<<endl;
            ...
            cout<<markednode<<": TEMP DELAY : "<<tempdelay<<" DELAY : "<<delay<<" difference "<<tempdelay - delay<<endl;        
            if(tempdelay>delay)
            {
...
//                    markednode=tempvect[i];
                //cout<<" removing node - "<<markednode<<" - markednode "<<g[markednode].type<<" - g[markednode].type"<<endl;
                    if(g[markednode].numreplacements==1)
                    {
                        ...
                        //g[markednode].cellCapacitance=capacitancemap[searchby];
                        g[markednode].cellCapacitance=g[markednode].cellCapacitance_old;

                        g[markednode].capacitance=g[markednode].capacitance_old;
                        g[markednode].delay=g[markednode].delay_old;
                        // g[markednode].slew[0]=g[markednode].slew_old[0];
                        // g[markednode].slew[1]=g[markednode].slew_old[1];
                        vector<int>::iterator it_slew;
                       for(it_slew=list_of_nodes_on_path.begin();it_slew!=list_of_nodes_on_path.end();++it_slew){
                            int i=*it_slew;
                            g[i].slew[0]=g[i].slew_old[0];
                            g[i].slew[1]=g[i].slew_old[1];
...
//                }
       ...
       // //cout<<"undid moves"<<undocount<<"totalmoves:"<<count<<"\n";
        ...
        for(path_itr=list_of_replaced_nodes.begin();path_itr!=list_of_replaced_nodes.end();++path_itr){
            //cout<<"---------------------------------------------------------------------------"<<endl;
            int current_node_on_path=*path_itr;
            cout<<"---------------------------------------------------------------------------"<<endl;
            //int current_node_on_path=*path_itr;
            ...
            cout<<"g["<<current_node_on_path<<"] capacitance -- "<<g[current_node_on_path].capacitance<<endl;
    ...
    ofstream myfile;
    myfile.open("replaced_gates_slew_round_req_26_08_04.txt",ios::out|ios::app);
