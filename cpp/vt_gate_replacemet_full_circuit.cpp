             ...
             else if(g[f].level==i && g[f].type!="PI" && g[f].type!="PO")
             {
                   // cout<<"g[f].level==i && g[f].type!=PI && g[f].type!=PO"<<endl;
                     for(intit=g[f].fanouts.begin();intit<g[f].fanouts.end();intit++)
                    {
                       ...
                       // cout<<" tempnode type :"<<g[tempnode].type<<" g[tempnode].slack :"<<g[tempnode].slack<<" g[tempnode].arrival :"<<g[tempnode].arrival<<"required"<<required<<endl;
...
//                        }
        ...
        sort(slackset.begin(),slackset.end(),mysortfunction);
        ofstream myfile;
        myfile.open("slackfile_full_circuit.txt",ios::out|ios::app);
        myfile<<"the nodes in the decreasing order of slack are\n";
        for(int i=0;i<slackset.size();i++)
        {
            int temp=slackset[i].first;
...
void sort_function_slack_full_graph(Graph &g,int num){
...
//            string choice=argv[6]; 
        ...
        double totalleakage=0;
            ...
            sort_function_slack_full_graph(g,num);
            // for(int j=0;j<vertex_count-1;j++)
            // {
...
//                }
    ...
    ofstream myfile;
    myfile.open("replaced_gates.txt",ios::out|ios::app);
