...
// slew calculation
    ...
    // //vector<double> lvt_index_rise_2{0.00300,0.006232,0.015536,0.038730,0.096549,0.240685,0.600000};
    // vector<double> hvt_index_2{0.00300,0.006232,0.015536,0.038730,0.096549,0.240685,0.600000};
    ...
    // vector<double> cell_rise_fall_index_hvt{0.00300,0.005552,0.012331,0.027386,0.060822,0.135080,0.300000};
  ...
  //          slew_calculation(lvt_index_1,index_2,lvt_risetransitionmap,lvt_falltransitionmap,1, templevel,num1,g);
            ...
            ofstream slew_lvt_file;
            slew_lvt_file.open("lvt_slew_file_cp_12_09_4.txt");
            for(int i=0;i<num1-1;i++)
            {
                ...
                slew_lvt_file<<g[i].slew[0]<<"\t"<<g[i].slew[1]<<endl;
            ...
            if(loadCapacitance == 0){
                ...
                if(number_of_fanouts == 1)
                    g[i].capacitance= 0.0027+loadCapacitance;
                      ...
                      g[i].capacitance=loadCapacitance+initial_net_capacitance_less_fanout+(number_of_fanouts - 1)*0.003;
                     //g[i].capacitance=loadCapacitance+initial_net_capacitance_less_fanout+(number_of_fanouts - 1)*0.004;
                else   
                    g[i].capacitance=loadCapacitance+initial_net_capacitance+(rf-1)*0.0015+9*0.003;
                    //g[i].capacitance=loadCapacitance+initial_net_capacitance+(number_of_fanouts-1)*0.001;
            ...
            // int number_of_fanouts=successor_map.size();
            // double initial_net_capacitance= 0.05;//0.028;
            // double initial_net_capacitance_less_fanout= 0.05;//0.028;
            // double decline_rate=0.000435;
            // int rf=  number_of_fanouts - 10;
            // if(loadCapacitance == 0){
            //      //g[i].capacitance=NET_DELAY_LVT;
            ...
            //     // g[i].capacitance=loadCapacitance+initial_net_capacitance+(number_of_fanouts - 1)*0.01;
            //     if(number_of_fanouts == 1)
            //         g[i].capacitance=loadCapacitance+ 0.028;
    ...
    if(pos !=string::npos){
        filename.replace(pos,4,".rise_cp_12_09_4_"+s_pi_r+"_"+s_pi_f);
    }
        ...
        myfile.close();
        ofstream slew_hvt_file;
        slew_hvt_file.open("hvt_slew_file_cp_12_09_4.txt");
        cout<<"======= Following are the list of Slews for different Node============="<<endl;
        for(int i=0;i<size;i++)
        {
            //if(i==26149 || i==2789|| i==8616|| i==8602|| i==132791|| i==97948|| i==97947|| i==97945|| i==97944|| i==9663 || i==9662 || i==163436 )
            ...
            slew_hvt_file<<g[i].slew[0]<<"\t"<<g[i].slew[1]<<endl;
