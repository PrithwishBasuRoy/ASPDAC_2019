...
void test_verilog_parser (string filename,string format) {
  ...
  set<std::string>::iterator myit;// this is an iterator that iterates through the set can be used for piset,poset or wireset
   ...
   map<int,string >celloutputmap;// if a wire (not pi not po) is in the output of a cell it gets added to this map. Both cellinput map and celloutputmap are useful for adding edges b/w cells
 typedef boost::adjacency_list<boost::vecS,boost::vecS,boost::directedS> Graph; // we intialize a graph ,for the time being it is bidirectional although making it uni-directional or a DAG will not make any difference because we are not adding any backward edges all the edges are forward (from PI->PO)
  ...
  int pi_po_count=0;// counts the pis and pos alone
  int elementcount=0;//counts total components pi+po+cells used as key for hash map mymap line no 1116
  string moduleName ;
...
//      cout << "Primary input: " << primaryInput << endl ;
    ...
    cout << "Primary input: " << primaryInput << endl ;
      ...
      mymap[elementcount]=primaryInput;// add it to the hash map and increment pi_po_count and elementcount
      mymap2[primaryInput]=elementcount;
      pi_po_count++;
      elementcount++;
      }
    }
  ...
  do {
    ...
    //  cout << "Primary output: " << primaryOutput << endl ;
      ...
      mymap[elementcount]=primaryOutput;// add it to the hash map mymap and increment pi_po_count and elementcount
      mymap2[primaryOutput]=elementcount;
      pi_po_count++;
      elementcount++;
    }
  ...
  do {
    ...
    Graph g;
  cout<<"total count"<<elementcount<<endl;
  cout<<"pi_po_count"<<pi_po_count<<endl;
...
//   cellcount++;
  ...
  do {
    ...
    string tempoutvect; // a vector that stores all the intermediate wires that are the cell's output
    valid = vp.read_cell_inst (cellType, cellInst, pinNetPairs) ;
    int temp=0;
    if (valid) {
      cout << cellType << " " << pi_po_count+cellcount << " "<<elementcount<<" " ;
      
      mymap[elementcount]=cellType;// we add the cellType to the hashmap instead of the cellInst as cellType will be useful for getting cell details like power area and timing during library parsing.
      mymap2[cellType]=elementcount;//find out where are u using this again
      
      int sizecount=pinNetPairs.size();
            
      for (int i=0; i < pinNetPairs.size(); ++i) {

          if(pinNetPairs[i].first=="ob")
              cout<<"It exists"<<endl;
          ...
          if(piset.find(pinNetPairs[i].second)!=piset.end()) // if the pin is the primary input set then
       {
          ...
          cout<<"primary input "<<pinNetPairs[i].second<<" "<<temp<<" ";
           ...
           boost::add_edge(temp,elementcount,g); // we add an incoming edge from PI to the cell
       }
       else if((pinNetPairs[i].first=="o"||pinNetPairs[i].first=="O" || pinNetPairs[i].first=="ob" || pinNetPairs[i].first=="Q" || pinNetPairs[i].first=="OB")  && (poset.find(pinNetPairs[i].second)!=poset.end())) // if the pin is the primary output set then
       {
//      mapit=mymap.begin();
            temp=mymap2[pinNetPairs[i].second];
          cout<<"primary output "<<pinNetPairs[i].second<<" "<<temp<<" ";

           boost::add_edge(elementcount,temp,g);// we add an from the cell to the PO
        
       }
       else if(wireset.find(pinNetPairs[i].second)!=wireset.end())// if it is an intermediate wire
      {
          if((pinNetPairs[i].first=="o" )||(pinNetPairs[i].first=="ob")||(pinNetPairs[i].first=="Q")||(pinNetPairs[i].first=="O") ||(pinNetPairs[i].first=="OB")){ // if it is a output of the cell
              
              tempoutvect=pinNetPairs[i].second; // we add it to the temporary outputvector
       ...
       cout << endl ;
       elementcount++;
   celloutputmap[pi_po_count+cellcount]=tempoutvect; // we map the temporary outputvector to the celloutputmap
  cellinputmap[pi_po_count+cellcount]=tempinvect;//we map the temporary inputvector the cellinputmap
  cellcount++;        
    }
  ...
  for(int j=0;j<cellcount;j++) // iterate over all the cells
{
    ...
    cout<<j<<"\t";
           invect=cellinputmap[pi_po_count+j];// get the input vectors for the cell
   
    for(int k=0;k<cellcount;k++)
    {
//       if(zeroes[j]!=cellinputmap[pi_po_count+j].size())
//       {

            if(std::find(invect.begin(),invect.end(),celloutputmap[pi_po_count+k])!=invect.end())//if input matches output
                {
                    boost::add_edge(pi_po_count+k,pi_po_count+j,g);// add edge b/w cell1 and cell2
                    cout<<"added edge b/w"<<pi_po_count+k<<"and"<<pi_po_count+j<<endl;
    //                zeroes[j]++;              
                }
     //   }
    }
...
if(format=="ispd")// we have two format options one is ispd and the other is hypergraph 
{
  ...
  * This is style 1 ispd benchmark format but multiple instances of same gate so have to use style 2*/
                /*       for(int i=0;i<pi_po_count+cellcount;i++)
{
    myfile<<mymap[i]<<" s 1\n";
    for(int j=0;j<pi_po_count+cellcount;j++)
    {
